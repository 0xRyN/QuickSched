...
typedef struct deque {
    ...
    // This should not be here, but it's easier than to create another struct
    int steal_success;
    int steal_fail;
} deque;
...
void log_steal_stats(scheduler *s) {
    int total_steal_success = 0;
    int total_steal_fail = 0;
    for (int i = 0; i < s->nthreads; i++) {
        total_steal_success += s->deques[i].steal_success;
        total_steal_fail += s->deques[i].steal_fail;
    }

    printf("Success : %i, Fail : %i\n", total_steal_success, total_steal_fail);
}

void log_task_stats(scheduler *s) {
    for (int i = 0; i < s->nthreads; i++) {
        printf("Thread %i - %i tasks\n", i, tasks_by_thread[i]);
    }
}
