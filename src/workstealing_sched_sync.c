...
void print_benchmark_tasks_by_thread(scheduler *s) {
    printf("Tasks done by thread: [");
    for (int i = 0; i < s->nthreads; i++) {
        printf("%d, ", tasks_by_thread[i]);
    }
    printf("]\n");
}
...
void check_active_threads(scheduler *s) {
    ...
    if (DEBUG) print_benchmark_tasks_by_thread(s);
    // Work is done, no active threads
...
scheduler *stealing_sched_init(int nthreads, int qlen, taskfunc f,
    ...
    pthread_cond_init(&s->work_done, NULL);
...
void wait_for_threads(scheduler *s) {
    pthread_mutex_lock(&s->lock);
